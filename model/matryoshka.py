...
class Matryoshka_CE_Loss(nn.Module):
	def __init__(self, relative_importance=None, is_functional=True, **kwargs):
		super(Matryoshka_CE_Loss, self).__init__()
		self.criterion = nn.CrossEntropyLoss(**kwargs) if not is_functional else F.binary_cross_entropy_with_logits
...
class MRL_Linear_Layer(nn.Module):
	...
	def forward(self, x):
		nesting_logits = ()
		for i, num_feat in enumerate(self.nesting_list):
			x_temp = x[:, :num_feat] if len(x.shape) == 2 else x[:, :, :num_feat]
			if self.efficient:
				if self.nesting_classifier_0.bias is None:
					nesting_logits+= (torch.matmul(x_temp, (self.nesting_classifier_0.weight[:, :num_feat]).t()), )
				else:
					nesting_logits+= (torch.matmul(x_temp, (self.nesting_classifier_0.weight[:, :num_feat]).t()) + self.nesting_classifier_0.bias, )
			else:
				nesting_logits +=  (getattr(self, f"nesting_classifier_{i}")(x_temp),)
...
class MRL_ReLU(nn.Module):
	def __init__(self, **kwargs):
		super(MRL_ReLU, self).__init__()
	
	def forward(self, x):
		return (F.relu(mrl_x) for mrl_x in x)