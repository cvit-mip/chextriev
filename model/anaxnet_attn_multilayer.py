...
class CustomModel(pl.LightningModule):
    def __init__(self, config, in_features=1024, hidden_features=1024, out_features=1024, num_classes=9, num_nodes=18):
        super().__init__()
        try:
            print(f'Anaxnet multilayer attention using global features is {config["is_global_feat"]}' + ' in a constrastive manner' if config['contrastive'] else '')
        except:
    ...
    def forward(self, images, y_node, y_graph, global_feat=None, return_loss=True, return_emb=False):
        ...
        if self.config['contrastive']:
            ...
            loss = loss + self.graph_importance * contrastive_loss
