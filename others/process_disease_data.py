...
from dataloader.anaxnet import DiseaseOcclusionDataset
...
parser.add_argument('--precise', action=argparse.BooleanOptionalAction, help='Precise masking in dataloader')
...
if __name__ == '__main__':
    ...
    store_dir = '/scratch/arihanth.srikar/dump_disease_data'
    store_dir += '_precise' if config['precise'] else ''
    ...
    try:
        ...
        occluded_emb = {consider_disease: np.load(f'{store_dir}/{config["run"]}/{config["task"]}_emb_{model_criteria}_{consider_disease.replace(" ", "_").replace("/", "-")}.npy') for consider_disease in nine_class_labels+["original"]}
        ...
        for model_criteria in ['auc']:
            ...
            for consider_disease in ["original"]+nine_class_labels:
                test_dataset = DiseaseOcclusionDataset(df, split='test', consider_disease=consider_disease, precise=config['precise'])
                test_loader = DataLoader(test_dataset, batch_size=config['batch_size'], shuffle=False, num_workers=config['num_workers'])
                print(f'Occulding anatomies for {consider_disease} from the test set using {model_criteria} model')
                ...
                consider_disease = consider_disease.replace(' ', '_').replace('/', '-')
                os.makedirs(store_dir, exist_ok=True)
                os.makedirs(f'{store_dir}/{config["run"]}', exist_ok=True)
                np.save(f'{store_dir}/{config["run"]}/{config["task"]}_emb_{model_criteria}_{consider_disease}.npy', all_emb)
                np.save(f'{store_dir}/{config["run"]}/{config["task"]}_labels_{model_criteria}_{consider_disease}.npy', all_labels)
                print(f'Saved {consider_disease} embeddings and labels')
    ...
    try:
        ...
        df = pd.DataFrame(columns=['consider_disease', 'disease', 'AP', 'HR', 'RR', 'retrieved_match', 'retrieved_similarity'])
        ...
        for k_disease, v_disease in occluded_emb.items():

            # build the index using faiss
            top_k = 10
            d = all_emb.shape[1]
            faiss_retriever = faiss.IndexFlatIP(d)
            faiss_retriever.add(v_disease)
            ...
            for _, (emb, query_labels) in enumerate(tqdm(zip(query_emb, all_labels), total=len(query_emb), desc=f'Retreiving top-{top_k} {k_disease}...')):
                ...
                # perform retrieval
                D, I = faiss_retriever.search(emb, top_k+1) if k_disease == "original" else faiss_retriever.search(emb, top_k)
                ...
                # labels = all_labels[I[:, 1:]] if k_disease == "original" else all_labels[I]
                # similarity = torch.tensor(D[:, 1:]) if k_disease == "original" else torch.tensor(D)
    ...
    t = PrettyTable(['Disease', 'Occluded for Disease', 'p_stat_0.05', 'p_stat_0.01', 'p_val'])
    # calculate stats for each anatomy
    for consider_disease in df['consider_disease'].unique():
        for d_name in nine_class_labels:
            t_stat, p_val = ttest_ind(df[(df['disease'] == d_name) & (df['consider_disease'] == consider_disease)]['AP'], df[(df['disease'] == d_name) & (df['consider_disease'] == 'original')]['AP'])
            p_005 = 'T' if p_val <= 0.05 else '-'
            p_001 = 'T' if p_val <= 0.01 else '-'
            t.add_row([d_name, consider_disease, p_005, p_001, f'{p_val:.3f}'])
        t.add_row(['', '', '', '', ''])
    print(t)
