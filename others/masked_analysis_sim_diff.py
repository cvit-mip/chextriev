...
def deletion_for_anatomy(
    ...
    print(f'Original Similarity: {orig_sim:.4f}, Sim: {sim:.4f}, Diff: {diff:.4f}, Anatomy: {anatomy}')
...
def exp(
    ...
    for x_off in tqdm(range(0, 224, stride)):
        lx, rx = x_off, min(x_off+window_size, 224)
        
        for y_off in range(0, 224, stride):
            ly, ry = y_off, min(y_off+window_size, 224)
            ...
            temp_img = np.zeros((224, 224))
            temp_img[lx:rx, ly:ry] = diff
            img_heat_map.append(torch.from_numpy(temp_img))

    img_heat_map = torch.stack(img_heat_map)
    img_heat_map = img_heat_map.mean(dim=0)
...
if __name__ == '__main__':
    ...
    # save image
    plt.savefig(f'results/query_retrieved_images_sim_diff_{query_id}.png')
    ...
    # save image
    plt.savefig(f'results/superimposed_heatmaps_sim_diff_{query_id}.png')


    # print('GT')
    # exp_images, exp_masks = [], []
    # for r_id, (retrived_id, lookup_dict) in enumerate(zip(retrieved_ids, lookup_dicts)):
    #     exp_img, exp_mask = exp(lookup_dict, best_model, retrived_id, df=df, consider_anatomies=anatomy_names, test_dataset=test_dataset)
    ...
    #     exp_img, exp_mask = exp(lookup_dict, anaxnet_model, retrived_id, df=df, consider_anatomies=anatomy_names, test_dataset=test_dataset)
    ...
    #     exp_img, exp_mask = exp(lookup_dict, resnet_model, retrived_id, df=df, consider_anatomies=anatomy_names, test_dataset=test_dataset, not_graph=True)
    ...
    #     exp_img, exp_mask = exp(lookup_dict, ath_model, retrived_id, df=df, consider_anatomies=anatomy_names, test_dataset=test_dataset, not_graph=True, transforms=ath_transform)
    ...
    # save image
    plt.savefig(f'results/final_heatmaps_sim_diff_{query_id}.png')
