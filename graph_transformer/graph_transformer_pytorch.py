...
class Attention(nn.Module):
    ...
    def forward(self, nodes, edges, mask = None, positions=None):
        h = self.heads
        
        if exists(self.pos_emb) and isinstance(self.pos_emb, nn.Embedding):
            # assert exists(positions), 'positions must be supplied if using absolute positional embedding'
            if exists(positions):
                pos_emb = self.pos_emb(positions.to(nodes.device))
                nodes = nodes + pos_emb
        ...
        if exists(self.pos_emb) and isinstance(self.pos_emb, RotaryEmbedding):
...
class GraphTransformer(nn.Module):
    def __init__(
        ...
        pos_emb = nn.Embedding(64, dim) if abs_pos_emb else pos_emb
        self.pos_emb = pos_emb
    ...
    def forward(
        ...
        if not exists(positions) and isinstance(self.pos_emb, nn.Embedding):
            positions = torch.arange(seq, device = nodes.device) if consider_positions else None

        all_edges = default(edges, 0) + default(adj_mat, 0)

        if residual_type == 2 or residual_type == 3:
            h_0 = nodes.clone()
        retrieved = h_0.clone()
        for attn_block, ff_block in self.layers:
            attn, attn_residual = attn_block
            if residual_type == 1 or residual_type == 3:
                nodes = attn_residual(attn(nodes, all_edges, mask = mask, positions = positions), nodes)
                if residual_type == 3:
                    nodes = attn_residual(nodes, h_0)
            elif residual_type == 2:
                nodes = attn_residual(attn(nodes, all_edges, mask = mask, positions = positions), h_0)
            ...
            if exists(ff_block):
                ...
                if residual_type == 1 or residual_type == 3:
                    ...
                    if residual_type == 3:
                        nodes = ff_residual(nodes, h_0)
                elif residual_type == 2:
                    nodes = ff_residual(ff(nodes), h_0)
